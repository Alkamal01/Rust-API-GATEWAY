//! Rusta API Gateway Server
//! 
//! High-performance HTTP API gateway built with Rust and Actix Web.
//! 
//! This binary provides the main server entry point for the Rusta gateway,
//! configuring and starting the HTTP server with all required middleware
//! and routing capabilities.

use rusta_rs::config::settings::load_settings;
use rusta_rs::config::validation::ConfigValidator;  
use rusta_rs::logs::logger::configure_logger;
use rusta_rs::middleware::security::security_headers;
use rusta_rs::middleware::rate_limit::AdvancedRateLimit;
use rusta_rs::models::settings::Settings;
use rusta_rs::routes::{auth_http, health, metrics, management, websocket};
use rusta_rs::services::http::RouteHandler;
use rusta_rs::services::metrics_store::MetricsStore;
use rusta_rs::services::websocket::WebSocketHandler;
...
async fn main() -> std::io::Result<()> {
    ...
    info!("Starting Rusta API Gateway v{}", config.version);
    ...
    let config_path = std::env::var("RUSTA_CONFIG_PATH")
    ...
    let host = std::env::var("RUSTA_HOST").unwrap_or_else(|_| "0.0.0.0".to_string());
    let port = std::env::var("RUSTA_PORT")
