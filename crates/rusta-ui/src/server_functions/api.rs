//! Server functions for calling the Rusta-gateway API.
//! 
...
/// Base URL for the Rusta Gateway API
...
pub async fn get_health() -> Result<HealthResponse, ServerFnError> {
    ...
    let response = reqwest::get(&url)
        ...
        .map_err(|e| {
            let msg = format!(
                "Failed to connect to Rusta Gateway at {}: {}. Make sure the gateway is running with: cargo run --bin Rusta-gateway",
                GATEWAY_BASE_URL, e
            );
