# Rusta UI - Admin Interface for Rusta Gateway 🎨

A modern, production-ready web-based admin interface for the Rusta API Gateway, built with **Leptos 0.8** and following Rust best practices.
...
1. **Rusta Gateway must be running** on port 5900:
   ```bash
...
Rusta_GATEWAY_URL=http://localhost:5900
...
This project adheres to the guidelines in `/rusta/llm.txt` and `/crates/Rusta-ui/llm.txt`:
...
**Solution:**
1. Ensure Rusta Gateway is running: `cargo run --bin Rusta-gateway`
...
- [Rusta Gateway Documentation](../../Readme.md)
