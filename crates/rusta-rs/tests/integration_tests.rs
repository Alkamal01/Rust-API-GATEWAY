//! Integration tests for the Rusta-rs gateway
//! 
...
use rusta_rs::middleware::auth::{JwtAuth, JwtConfig, Claims, create_test_token};
use rusta_rs::routes::{health, metrics};
use rusta_rs::services::circuit_breaker::{CircuitBreaker, CircuitBreakerConfig};
...
async fn test_metrics_endpoint() {
    ...
    assert!(body_str.contains("Rusta_requests_total"));
    assert!(body_str.contains("Rusta_requests_success_total"));
    assert!(body_str.contains("Rusta_requests_error_total"));
    assert!(body_str.contains("Rusta_response_time_avg"));
}
...
async fn test_jwt_authentication_success() {
    ...
    let config = JwtConfig::new(secret.to_string())
        .with_issuer("Rusta-rs".to_string())
...
async fn test_cors_headers() {
    use rusta_rs::middleware::security::cors_headers;
