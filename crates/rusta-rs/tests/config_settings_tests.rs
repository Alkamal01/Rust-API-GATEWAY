...
use rusta_rs::models::router::{Router, Backend};
use rusta_rs::models::router::Protocol;
use std::env;
use std::io::Write;
...
fn test_load_settings_with_environment_variable() {
    ...
    let original_path = env::var("Rusta_CONFIG_PATH").ok();
    
    let settings = create_test_settings();
    let temp_file = create_config_file(&settings);

    // Test by setting environment variable to our test file
    env::set_var("Rusta_CONFIG_PATH", temp_file.path());
    
    let result = load_settings();
    
    // Restore original environment variable if it existed
    if let Some(path) = original_path {
        env::set_var("Rusta_CONFIG_PATH", path);
    } else {
        env::remove_var("Rusta_CONFIG_PATH");
    }
...
fn test_load_settings_file_not_found() {
    env::set_var("Rusta_CONFIG_PATH", "./nonexistent.json");
...
fn test_load_settings_path_traversal_protection() {
    ...
    env::set_var("Rusta_CONFIG_PATH", &malicious_path);
