...
use rusta_rs::routes::{http, metrics};
...
async fn test_real_metrics_collection() {
    ...
    assert!(metrics_text.contains("Rusta_requests_total 5"));
    assert!(metrics_text.contains("Rusta_requests_error_total 5")); // All failed due to no upstream
}