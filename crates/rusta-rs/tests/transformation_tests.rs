use rusta_rs::middleware::transform::*;
...
fn test_request_header_add() {
    ...
    assert_eq!(headers.get("X-Gateway").unwrap().to_str().unwrap(), "Rusta-rs");
...
fn test_request_header_replace_regex() {
    let config = RequestTransformation {
        headers: vec![HeaderTransformation {
            ...
            replacement: Some("RustaGateway/$1".to_string()),
        }],
        path: None,
        query_params: vec![],
    };
    ...
    assert_eq!(headers.get(USER_AGENT).unwrap().to_str().unwrap(), "RustaGateway/5.0 (compatible)");
}
...
fn test_response_header_transformation() {
    ...
    assert_eq!(headers.get("X-Powered-By").unwrap().to_str().unwrap(), "Rusta Gateway");
