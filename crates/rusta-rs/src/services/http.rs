...
/// High-performance HTTP request handler for the Rusta-rs gateway.
/// 
...
/// use rusta_rs::models::router::{Router, Backend, Protocol};
/// 
/// let routes = vec![
///     Router {
...
impl RouteHandler {
    ...
    /// 
    /// ## Added Headers
    /// - `user-agent` - Default "Rusta-rs/0.2.0" if not present
    /// 
    /// # Performance Optimizations
    /// 
    ...
    fn build_headers_optimized(
        ...
        reqwest_headers.entry("user-agent")
            .or_insert_with(|| HeaderValue::from_static("Rusta-rs/0.2.0"));
